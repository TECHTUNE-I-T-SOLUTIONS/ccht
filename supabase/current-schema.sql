...
CREATE TABLE public.aspirant_profiles (
  ...
  application_status text NOT NULL DEFAULT 'draft'::text CHECK (application_status = ANY (ARRAY['draft'::text, 'pending'::text, 'incomplete'::text, 'under_review'::text, 'correction_required'::text, 'accepted'::text, 'pending_payment'::text, 'admitted'::text, 'admission_accepted'::text, 'migrated'::text, 'student'::text, 'rejected'::text])),
  current_stage text NOT NULL DEFAULT 'signup'::text CHECK (current_stage = ANY (ARRAY['signup'::text, 'payment'::text, 'documents'::text, 'exam'::text, 'admission_fee'::text, 'migration'::text, 'admission_acceptance'::text, 'completed'::text])),
...
CREATE TABLE public.selected_courses (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  student_id uuid NOT NULL,
  course_id uuid NOT NULL,
  enrollment_id uuid,
  session character varying NOT NULL DEFAULT '2026/2027'::character varying,
  semester character varying NOT NULL DEFAULT 'first'::character varying,
  ...
  CONSTRAINT selected_courses_pkey PRIMARY KEY (id),
  CONSTRAINT selected_courses_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.profiles(id),
  CONSTRAINT selected_courses_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id),
  CONSTRAINT selected_courses_enrollment_id_fkey FOREIGN KEY (enrollment_id) REFERENCES public.enrollments(id),
  CONSTRAINT selected_courses_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES public.admin_profiles(profile_id)
);
CREATE TABLE public.student_exam_sessions (
  ...
  exam_description text,
  exam_type character varying NOT NULL DEFAULT 'regular'::character varying CHECK (exam_type::text = ANY (ARRAY['regular'::character varying, 'mid_semester'::character varying, 'final'::character varying, 'supplementary'::character varying, 'resit'::character varying]::text[])),
  ...
  CONSTRAINT student_exam_sessions_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id),
  CONSTRAINT student_exam_sessions_session_id_fkey FOREIGN KEY (session_id) REFERENCES public.academic_sessions(id),
  CONSTRAINT student_exam_sessions_semester_id_fkey FOREIGN KEY (semester_id) REFERENCES public.academic_semesters(id),
  CONSTRAINT student_exam_sessions_published_by_fkey FOREIGN KEY (published_by) REFERENCES public.admin_profiles(profile_id),
  CONSTRAINT student_exam_sessions_proctoring_config_fkey FOREIGN KEY (proctoring_config_id) REFERENCES public.exam_proctoring_config(id)
);
CREATE TABLE public.student_exam_questions (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  exam_session_id uuid NOT NULL,
  question_text text NOT NULL,
  question_type character varying NOT NULL DEFAULT 'multiple_choice'::character varying CHECK (question_type::text = ANY (ARRAY['multiple_choice'::character varying, 'true_false'::character varying, 'short_answer'::character varying, 'essay'::character varying, 'fill_blank'::character varying]::text[])),
  ...
  CONSTRAINT student_exam_questions_exam_session_id_fkey FOREIGN KEY (exam_session_id) REFERENCES public.student_exam_sessions(id)
);
CREATE TABLE public.student_exam_attempts (
  ...
  status character varying NOT NULL DEFAULT 'not_started'::character varying CHECK (status::text = ANY (ARRAY['not_started'::character varying, 'in_progress'::character varying, 'submitted'::character varying, 'auto_submitted'::character varying, 'abandoned'::character varying, 'graded'::character varying]::text[])),
  ...
  CONSTRAINT student_exam_attempts_exam_session_id_fkey FOREIGN KEY (exam_session_id) REFERENCES public.student_exam_sessions(id),
  CONSTRAINT student_exam_attempts_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.profiles(id),
  CONSTRAINT student_exam_attempts_enrollment_id_fkey FOREIGN KEY (enrollment_id) REFERENCES public.enrollments(id),
  CONSTRAINT student_exam_attempts_graded_by_fkey FOREIGN KEY (graded_by) REFERENCES public.teacher_profiles(profile_id)
);
CREATE TABLE public.student_exam_answers (
  ...
  CONSTRAINT student_exam_answers_exam_attempt_id_fkey FOREIGN KEY (exam_attempt_id) REFERENCES public.student_exam_attempts(id),
  CONSTRAINT student_exam_answers_question_id_fkey FOREIGN KEY (question_id) REFERENCES public.student_exam_questions(id)
);
CREATE TABLE public.student_exam_violations (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  exam_attempt_id uuid NOT NULL,
  violation_type character varying NOT NULL CHECK (violation_type::text = ANY (ARRAY['tab_switch'::character varying, 'visibility_change'::character varying, 'copy_paste'::character varying, 'right_click'::character varying, 'devtools_open'::character varying, 'multiple_faces'::character varying, 'no_face'::character varying, 'suspicious_movement'::character varying, 'audio_detected'::character varying, 'unauthorized_device'::character varying, 'time_limit_exceeded'::character varying, 'other'::character varying]::text[])),
  severity character varying NOT NULL DEFAULT 'warning'::character varying CHECK (severity::text = ANY (ARRAY['info'::character varying, 'warning'::character varying, 'severe'::character varying, 'critical'::character varying]::text[])),
  description text,
  screenshot_url text,
  metadata jsonb,
  ...
  CONSTRAINT student_exam_violations_exam_attempt_id_fkey FOREIGN KEY (exam_attempt_id) REFERENCES public.student_exam_attempts(id),
  CONSTRAINT student_exam_violations_acknowledged_by_fkey FOREIGN KEY (acknowledged_by) REFERENCES public.admin_profiles(profile_id)
);
CREATE TABLE public.student_exam_recordings (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  exam_attempt_id uuid NOT NULL,
  recording_type character varying NOT NULL CHECK (recording_type::text = ANY (ARRAY['video'::character varying, 'audio'::character varying, 'screen'::character varying, 'webcam'::character varying]::text[])),
  ...
  status character varying NOT NULL DEFAULT 'processing'::character varying CHECK (status::text = ANY (ARRAY['processing'::character varying, 'completed'::character varying, 'failed'::character varying, 'deleted'::character varying]::text[])),
  metadata jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT student_exam_recordings_pkey PRIMARY KEY (id),
  CONSTRAINT student_exam_recordings_exam_attempt_id_fkey FOREIGN KEY (exam_attempt_id) REFERENCES public.student_exam_attempts(id)
);
CREATE TABLE public.student_exam_feedback (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  exam_attempt_id uuid NOT NULL,
  student_id uuid NOT NULL,
