  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      if ((data?.user?.role || role) === 'aspirant') {
        const [photoRes, notificationRes] = await Promise.all([fetch('/api/v1/admissions/profile-photo'), fetch('/api/v1/notifications/aspirant')])
  ...
  const navItems = useMemo(() => {
    const roleItems = {
      ...
      aspirant: [
        ...
        { label: 'Exam', href: '/aspirant/exam', icon: ShieldCheck },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex min-h-[calc(100vh-64px)]">
        <aside
          className={`fixed inset-y-0 left-0 z-30 border-r border-border bg-white/95 backdrop-blur transition-all duration-300 dark:bg-slate-950/95 xl:fixed xl:top-16 xl:h-[calc(100vh-4rem)] ${sidebarOpen ? 'translate-x-0' : '-translate-x-full xl:translate-x-0'} ${sidebarCollapsed ? 'w-[4.75rem]' : 'w-[18rem]'}`}
        >
          <button type="button" onClick={() => setSidebarCollapsed((value) => !value)} className={`absolute -right-3 top-4 z-50 flex h-8 w-8 items-center justify-center rounded-full border-2 border-border bg-white shadow-md transition-all hover:border-primary hover:text-primary dark:bg-slate-900 xl:flex`} aria-label="Collapse sidebar">
        ...
        {sidebarOpen && <div className="fixed inset-0 z-20 bg-white/50 dark:bg-slate-950/60 xl:hidden" onClick={() => setSidebarOpen(false)} />}
